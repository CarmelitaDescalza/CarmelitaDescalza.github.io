...
if ( ! function_exists( 'minimal_blocks_posted_on' ) ) :
	/**
	 * Prints HTML with meta information for the current post-date/time and author.
	 */
	function minimal_blocks_posted_on() {
        global $post;
		...
		if(is_single()){
            ...
            $archive_month = get_the_time('m'); 
            $archive_day   = get_the_time('d'); 
            $posted_on = '<a href="' . esc_url( get_day_link( $archive_year, $archive_month, $archive_day) ) . '" rel="bookmark">' . $time_string . '</a>';
        }
		...
		if(is_single()){
            echo '<span class="entry-meta posted-on">' . $posted_on . '</span><span class="byline"> ' . $byline . '</span>'; // WPCS: XSS OK.
        }else{
            ...
            if ( $categories_list ) {
                ...
                echo '<span class="cat-links"><span class="thememattic-icon ion-ios-folder-outline"></span>' . $categories_list . '</span>'; // WPCS: XSS OK.
            }
        }
	}
endif;
...
if (!function_exists('minimal_blocks_posted_date_only')) :
    ...
    function minimal_blocks_posted_date_only()
    {
        ...
        echo '<span class="entry-meta posted-on">' . $posted_on . '</span>'; // WPCS: XSS OK.
    }
endif;
    ...
    function minimal_blocks_entry_category()
    {
        ...
        if ('post' === get_post_type()) {
            ...
            if ($categories_list && minimal_blocks_categorized_blog()) {
                echo "<span class='entry-meta meta-categories'>".$categories_list."</span>";
            }
        }
    }
endif;
...
if (!function_exists('minimal_blocks_posted_comment')) :
    function minimal_blocks_posted_comment()
    {
        if (!is_single() && !post_password_required() && (comments_open() || get_comments_number())) {
            ...
            comments_popup_link( ' ', esc_html__( '1 Comment', 'minimal-blocks' ), esc_html__( '% Comments', 'minimal-blocks' ) );
            echo '</span>';
        } 
    }
endif;
	...
	function minimal_blocks_entry_footer() {
		...
		if ( 'post' === get_post_type() ) {
			...
			if ( $tags_list ) {
				...
				echo '<span class="tags-links"><span class="thememattic-icon ion-ios-pricetags-outline"></span>' . $tags_list . '</span>'; // WPCS: XSS OK.
			}
		}
            ...
            edit_post_link(
                sprintf(
                    ...
                    /* translators: %s: Name of current post. Only visible to screen readers */
                        __( 'Edit <span class="screen-reader-text">%s</span>', 'minimal-blocks' ),
                        array(
                            'span' => array(
                                'class' => array(),
                            ),
